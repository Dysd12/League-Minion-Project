...
bool nRF24_transmit(const uint8_t *data) {
    /* 1. Write payload to TX FIFO on nRF24 module. */
    nRF24_write(W_TX_PAYLOAD, data, 2);

    /* 2. Flash CE to transmit package. Increasing delay transmits remaining packages in FIFO. */
    set_ce(1);
    //delay_ms(1);
    set_ce(0); 

    /* 3. Read STATUS register to determine if package was transmitted. */
    uint8_t status_reg = 0;
    nRF24_read(STATUS, &status_reg, 1);
    if(status_reg & STATUS_TX_DS) {
        nRF24_write(STATUS, (uint8_t[]){STATUS_TX_DS}, 1);
        nRF24_cmd(FLUSH_TX); // Clear TX FIFO
        return 1;
    }
    return 0;
}
